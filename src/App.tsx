...
const listOfTodo = () => {
  ...
  const user = usersFromServer.find(user => todo.userId === user.id)
  return {
    ...todo,
    user
  }
...
export const App = () => {
  ...
  const maxId = historyChange.length > 0 ? Math.max(...historyChange.map(todo => todo.id)) : 0;

  const uniqueUsers = usersFromServer.reduce((acc, item) => {
  ...
  return (
    <div className="App">
      ...
      <form action="/api/todos" method="POST">
        ...
        <div className="field">
          <select data-cy="userSelect" value={selectUser} onChange={handleSelectUser}>
